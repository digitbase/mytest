...
var search = function (){
	...
	var res_div = $("#res_div");
    var url = "./searchyq.php";
    if (keyWord.length > 0) {
        
		$.get(url, { word: keyWord},
		function(data){
			if (data.error ==1) {
				let ar = (data.data.info);

				let tb = $("<table class='table dataTable table-striped table-bordered table-hover'>");
				let tr = $('<tr class="t_tr"><td width="80px">字</td><td>词</td><td>常用构式</td><td>词义</td></tr>');
                ...
                for (let i=0; i <ar.length; i++) {
                	...
                	let th = $('<tr>');
                	let td1= $('<td class="t_tr">').html(ar[i]['f2']);
                	let td2= $('<td width="40%">').html(ar[i]['f3']);
                	let td3= $('<td width="20%">').html(ar[i]['f4']);
                	let td4= $('<td>').html(ar[i]['f5']);

                	th.append(td1).append(td2).append(td3).append(td4);
                	tb.append(th);
                }
                $('#res_div').append(tb);
