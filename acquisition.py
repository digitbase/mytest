...
def process(logger, data_source_id, host, port):
    ...
    while True:
        ...
        if rows_point is None or len(rows_point) == 0:
            ...
            # sleep 3 seconds and go back to the begin of outermost while loop to reload points
            time.sleep(3)
            continue
        ...
        client = ModbusTcpClient(host, port, framer=ModbusFramer)
        client.connect()
        Defaults.Timeout = 10
        ...
        while True:
            ...
            for point in point_list:
                ...
                try:
                    ...
                    response = client.read_holding_registers(address=address['offset'],count=1, unit=address['slave_id'])  
