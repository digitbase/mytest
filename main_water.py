...
def acquisitionwater(logger, data_source_id, host, port):
    print("row_data_source=%s,  server['host']=%s, server['port']=%s " % (data_source_id, host, port))
    pass
...
def main():
    ...
    while True:
        ...
        try:
            query = (" SELECT ds.id, ds.name, ds.connection "
                     ...
                     " WHERE  ds.gateway_id = g.id AND g.id = 2 "
                     " ORDER BY ds.id ")
    ...
    for row_data_source in rows_data_source:
        ...
        acquisitionwater(logger, row_data_source[0], server['host'], server['port'])
